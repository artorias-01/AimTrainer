      ...
      <div
        className={`relative z-10 w-full transition-all duration-200 ${
          showDrillMenu
            ...
            : 'max-w-md text-center space-y-6 bg-[#0d0d0d]/60 backdrop-blur-md p-6 sm:p-8 rounded-[16px] border border-[#262626] shadow-2xl'
        }`}
      >
        <AnimatePresence mode="wait">
          {showDrillMenu ? (
            ...
            >
              ...
              <div className="bg-[#0d0d0d]/85 backdrop-blur-md p-6 rounded-[16px] border border-[#262626] shadow-2xl space-y-5 w-full md:w-72 shrink-0 flex flex-col justify-between text-left">
                ...
                <button
                  ...
                  className="btn-editorial-secondary py-3 text-xs font-bold uppercase flex items-center justify-center gap-2 text-white border-[#262626] hover:border-accent transition-all active:scale-[0.98] focus:outline-none focus-visible:outline-accent"
                >
              ...
              <div className="flex-1 w-full max-w-full overflow-hidden bg-[#0d0d0d]/90 backdrop-blur-md border border-[#262626] rounded-[16px] p-6 space-y-4 shadow-2xl text-left">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        onMouseEnter={() => soundManager.playHover()}
                        title="Scroll Left"
                        className="p-1.5 rounded-[8px] bg-[#141414] hover:bg-accent/20 border border-[#262626] hover:border-accent text-neutral-300 hover:text-accent transition-all active:scale-95 focus:outline-none focus-visible:outline-accent"
                      >
                ...
                >
                  {allScenarios.map((sc) => (
                    <button
                      ...
                      onMouseEnter={() => soundManager.playHover()}
                      className="w-64 h-52 shrink-0 bg-[#121212] hover:bg-accent/10 border border-[#262626] hover:border-accent rounded-[12px] p-4 flex flex-col justify-between text-left transition-all duration-150 group focus:outline-none focus-visible:outline-accent hover:-translate-y-1 active:scale-[0.98] shadow-md"
                    >
            ...
            >
              ...
              {/* Main Menu Editorial Button Cards */}
                ...
                {menuOptions.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={opt.onClick}
                      onMouseEnter={() => soundManager.playHover()}
                      className={`group relative w-full px-5 py-3.5 rounded-[12px] font-display font-bold text-sm tracking-wider flex items-center justify-between border transition-all duration-150 active:scale-[0.98] focus:outline-none focus-visible:outline-accent ${
                        opt.isPrimary
                          ? 'bg-accent/15 border-accent text-white hover:bg-accent/25 hover:border-accent shadow-lg shadow-accent/10'
                          : 'bg-[#121212]/90 border-[#262626] text-neutral-200 hover:text-white hover:border-accent/60 hover:bg-[#181818]'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-[8px] flex items-center justify-center transition-colors ${
                            opt.isPrimary
                              ...
                              : 'bg-[#1a1a1a] text-accent group-hover:bg-accent group-hover:text-[#0d0d0d]'
                          }`}
                        >
                          <IconComponent className="w-4 h-4" />
                        </div>
                        <span className="uppercase text-left">{opt.label}</span>
                      </div>

                      <ArrowRight className="w-4 h-4 text-accent group-hover:translate-x-1.5 transition-transform duration-150" />
                    </button>
                  );
                })}
              </div>
