...
import { Sliders, Crosshair, Check, Volume2, VolumeX, Keyboard, User, Download, Copy, Trash2, CheckCircle2, Gauge, AlertCircle, Cpu, Save, Plus, Zap } from 'lucide-react';
...
export const SettingsPage: React.FC = () => {
  ...
  const handleImportJsonCode = () => {
    ...
    if (parsed) {
      ...
      setHexError(null);
      setImportStatusMsg('SUCCESSFULLY IMPORTED CROSSHAIR PRESET!');
      setImportCodeInput('');
      showSaveConfirmation('CROSSHAIR IMPORTED');
    } else {
      setImportStatusMsg('INVALID JSON CROSSHAIR BLOB. TRY AGAIN.');
    }
  ...
  return (
    <motion.div
      ...
      onKeyDown={listeningKeyFor ? handleKeyDownRebind : undefined}
      tabIndex={0}
      className="w-full bg-[#0d0d0d] text-white min-h-screen py-12 px-6 md:px-16 space-y-12 focus:outline-none relative select-none"
    >
      ...
      <div className="max-w-7xl mx-auto space-y-4 border-b border-[#262626] pb-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 font-mono text-xs text-[#f5b8c9] uppercase tracking-widest">
            ...
            SYSTEM CONFIGURATION // PLAYER PREFERENCES
          </div>
          <button
            onClick={() => setShowClearConfirm(true)}
            className="px-3 py-1.5 rounded-[12px] bg-[#1a1a1a] hover:bg-red-950/40 text-neutral-400 hover:text-red-400 border border-[#262626] hover:border-red-900/50 text-xs font-mono font-bold flex items-center gap-2 transition-all"
          >
        ...
        <p className="font-sans-ui text-neutral-400 text-sm max-w-xl">
          Configure mouse sensitivity scaling, DPI cm/360 conversions, custom crosshairs, hex color codes, target pacing multipliers, audio levels, and keybinds.
        </p>
      </div>

      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Left Column: Profile, Sensitivity, Target Pacing & Audio */}
        <div className="space-y-8">
          ...
          <div className="bg-[#141414] border border-[#262626] rounded-[12px] p-8 space-y-4">
            <div className="flex items-center justify-between border-b border-[#262626] pb-4">
              <h3 className="font-mono text-xs text-[#f5b8c9] uppercase tracking-widest flex items-center gap-2">
                <Cpu className="w-4 h-4 text-[#f5b8c9]" /> PERFORMANCE MODE (LOW-END HARDWARE)
              </h3>
              <button
                onClick={() => {
                  togglePerformanceMode();
        ...
        {/* Right Column: Custom Crosshair & Keybinds */}
        <div className="space-y-8">
          {/* Custom Crosshair Studio */}
          <div className="bg-[#141414] border border-[#262626] rounded-[12px] p-8 space-y-6">
            <div className="flex items-center justify-between border-b border-[#262626] pb-4">
              <h3 className="font-mono text-xs text-[#f5b8c9] uppercase tracking-widest flex items-center gap-2">
            ...
            {/* PART 1: Custom Hex Code Color Input */}
            <div className="space-y-3 bg-[#0d0d0d] p-5 rounded-[12px] border border-[#262626]">
              <label className="text-[10px] font-mono text-[#f5b8c9] uppercase tracking-widest block">
                LINE COLOR (HEX CODE)
              </label>
          ...
          {/* Keybinds Rebinding Section */}
          <div className="bg-[#141414] border border-[#262626] rounded-[12px] p-8 space-y-6">
            <div className="flex items-center justify-between border-b border-[#262626] pb-4">
              <h3 className="font-mono text-xs text-[#f5b8c9] uppercase tracking-widest flex items-center gap-2">
                <Keyboard className="w-4 h-4 text-[#f5b8c9]" /> KEYBIND REBINDING STUDIO
              </h3>
